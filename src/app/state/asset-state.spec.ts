...
describe('AssetState', () => {
    ...
    beforeEach(() => {
        ...
        service.getAssetList = jest.fn(() => {
            console.log("mock asset list")
            return of<any>(actualAssetList)
        });
    });



    it('it retuns asset list', () => {
        store.dispatch(new FetchAllAssetAction());
        const assetList = store.selectSnapshot(state => state.assets.assetList);
